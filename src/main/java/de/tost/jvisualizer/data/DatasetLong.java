...
public class DatasetLong {
    ...
    private long calculatedMin, calculatedMax;


    public DatasetLong(int slotSize) {
    ...
    public long get(int elementIndex) {
    ...
    public long getCalculatedMin(){
        return calculatedMin;
    }

    public long getCalculatedMax(){
        return calculatedMax;
    }

    public long[] getArray(int firstIndex, int lastIndex) {
    ...
    public void fillVertexBuffer(VertexBuffer buffer, int divisionFactor) {
        float[] data = new float[getSize()];
        for (int i = 0; i < getSize(); i++) {
            if (divisionFactor <= 1) {
                data[i] = get(i);
            } else {
                data[i] = get(i) / (float) divisionFactor;
            }
        }
